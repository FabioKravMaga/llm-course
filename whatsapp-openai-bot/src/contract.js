...
export async function buildContractPdf(lead) {
  ...
  await new Promise((res, rej) => {
    ...
    clause(doc, 'CLÁUSULA 1ª — DO OBJETO',
      `O CONTRATADO prestará serviços advocatícios ao CONTRATANTE, compreendendo: ${
        lead.scope || lead.case_summary || 'consultoria e patrocínio da causa descrita pelo CONTRATANTE.'
      }`
    );

    clause(doc, 'CLÁUSULA 2ª — DOS HONORÁRIOS',
      `Pelos serviços prestados, o CONTRATANTE pagará ao CONTRATADO o valor total de ${formatBRL(lead.fee_amount)}, ` +
      `na seguinte forma: ${lead.payment_terms || 'a combinar entre as partes'}.` +
      (config.lawyer.pixKey ? ` Chave PIX para pagamentos: ${config.lawyer.pixKey}.` : '')
    );
    ...
    clause(doc, 'CLÁUSULA 3ª — DAS OBRIGAÇÕES',
      ...
      'documentos e informações verídicas, bem como a pagar os honorários nas condições acima.'
    );
    ...
    doc.moveDown(2);
    doc.text(`${(config.lawyer.address || '').split(',')[0] || 'Local'}, ${todayBR()}.`, { align: 'right' });
    doc.moveDown(3);

    sigLine(doc, 'CONTRATANTE', lead.client_name || '');
    doc.moveDown(2);
    sigLine(doc, 'CONTRATADO', `${config.lawyer.name}${config.lawyer.oab ? ` — OAB ${config.lawyer.oab}` : ''}`);
